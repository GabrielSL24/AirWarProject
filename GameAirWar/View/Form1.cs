...
using System.IO.Ports; //Para usar el arduino, puerto COM
...
namespace GameAirWar
{
    public partial class Form1 : Form
    {
        ...
        private string UltimoComandoDeDireccion = "CINTA_OFF"; // Estado inicial
        private Timer comandoTimer;
        private string comandoPendiente = ""; // Almacena el comando pendiente para enviar
        ...
        public Form1()
        {
            ...
            serialPort.Open();
            Console.WriteLine("Puerto COM abierto.");

            // Configurar temporizador para enviar comandos al Arduino
            comandoTimer = new Timer();
            comandoTimer.Interval = 100; // Intervalo de envío (100 ms)
            comandoTimer.Tick += ComandoTimer_Tick;
            comandoTimer.Start();
            ...
            // Agregar un evento al cerrar la aplicación
        ...
        private void OnFormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                // Enviar comando al Arduino para detener los servos
                if (serialPort.IsOpen)
                {
                    serialPort.WriteLine("CINTA_OFF"); // Comando para detener
                    Console.WriteLine("Comando enviado: CINTA_OFF");
            ...
            {
                Console.WriteLine($"Error al cerrar el puerto COM: {ex.Message}");
            }
        }
        ...
        private string ultimoComandoEnviado = ""; // Almacena el último comando enviado

        private void GunTimer_Tick(object sender, EventArgs e)
        {
            var graph = map.GetGraph() as GraphList;

            string nuevoComando = ""; // Comando a enviar al Arduino

            if (gun.X >= MapPictureBox.Width)
            {
                gun.Speed = -Math.Abs(gun.Speed);
                nuevoComando = "CINTA_REVERSA";
            }
            else if (gun.X <= 0)
            {
                gun.Speed = Math.Abs(gun.Speed);
                nuevoComando = "CINTA_ON";
            }

            // Enviar el comando solo si es diferente al último enviado
            if (!string.IsNullOrEmpty(nuevoComando) && nuevoComando != ultimoComandoEnviado)
            {
                serialPort.WriteLine(nuevoComando);
                Console.WriteLine($"Comando enviado: {nuevoComando}");
                ultimoComandoEnviado = nuevoComando; // Actualizar el comando enviado
            }
        ...
        private void ComandoTimer_Tick(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(comandoPendiente) && comandoPendiente != UltimoComandoDeDireccion)
            {
                // Enviar el comando pendiente
                serialPort.WriteLine(comandoPendiente);
                Console.WriteLine($"Comando enviado: {comandoPendiente}");
                UltimoComandoDeDireccion = comandoPendiente; // Actualizar el último comando enviado
                comandoPendiente = ""; // Limpiar comando pendiente
            }
        }
        ...
        private void MapPictureBox_MouseDown(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                ...
                //Se prende el led cada ve que dispara:
                serialPort.WriteLine("Disparo");
            }
        }
    }
}
